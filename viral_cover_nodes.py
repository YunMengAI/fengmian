...
def build_user_prompt(封面风格, 主题关键词, 封面标题, 自定义要求, has_image=False):
    ...
    style_notice = (
        "封面风格选择为“自定义”，请优先读取自定义要求里的风格描述；如果自定义要求没有写风格，就按高点击率商业封面风处理。"
        if 封面风格 == "自定义"
        else "请严格按照用户选择的封面风格执行；如果该风格和自定义要求冲突，以自定义要求为准。"
    )
...
如果没有图片，请直接根据主题关键词、封面标题、封面风格和补充要求生成适合文生图的封面提示词。输出时只给最终提示词，不要解释过程。"""
